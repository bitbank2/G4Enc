//
//  G4 Encoder Test
//  Created by Larry Bank Feb 5 2025
//
#include "../../../src/G4ENCODER.cpp" // include it like a header file
#include "bart_tif.h"
...
//    uint8_t *pFuzzData;
    ...
    if (rc == G4ENC_SUCCESS) {
        ...
        if (rc == G4ENC_DATA_OVERFLOW) {
            ...
            printf("Worst case output didn't overflow - something went wrong\n");
        }
    } else {
        TIFFLOG(__LINE__, szTestName, " - FAILED");
        printf("g4.init() returned %d\n", rc);
    }
    ...
    *(uint32_t *)&ucTemp[sizeof(ucTemp)/2] = 0x12345678; // test pattern
    ...
    if (rc == G4ENC_SUCCESS) {
        ...
        if (rc == G4ENC_IMAGE_COMPLETE && iSize == sizeof(bart_tif) && memcmp(ucTemp, bart_tif, iSize) == 0) {
            ...
            if (iSize != sizeof(bart_tif))
                printf("Output size doesn't match\n");
            else
                printf("Output data doesn't match");
        }
    } else {
        TIFFLOG(__LINE__, szTestName, " - FAILED");
        printf("g4.init() returned %d\n", rc);
    ...
    if (rc == G4ENC_SUCCESS) {
        ...
        if (memcmp(&ucPixels[0x92], "Created with G4ENCODER by Larry Bank", 36) != 0) { // check creator message
            y = 0;
            printf("TIFF Creator tag has been modified.\n");
        }
        if (rc == G4ENC_IMAGE_COMPLETE && iSize == 183 && y == 1) {
            ...
            TIFFLOG(__LINE__, szTestName, " - FAILED");
            if (iSize != 4+(11*12))
                printf("TIFF header size doesn't match\n");
            else
                printf("Invalid TIFF header data");
        }
    } else {
        TIFFLOG(__LINE__, szTestName, " - FAILED");
        printf("g4.init() returned %d\n", rc);
